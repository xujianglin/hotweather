...
public class ChooseAreaActivity extends Activity {
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			isFromWeatherActivity=getIntent().getBooleanExtra("from_weather_activity", false);
					...
					//已经选择了城市且不是从WeatherActivity跳转过来，才会直接跳装到WeatherActivity
		...
		/*
		 * 查询选中省内所有的市，优先从数据库查询，如果没有查询到再去数据库查询
		 */
			...
			HttpUtil.sendHttpRequest(address, new HttpCallbackListener(){
				...
				public void onFinish(String response) {
					...
					if("province".equals(type)){
						...
						result=Utility.handleCouniesResponse(coolWeatherDB, response, selectedCity.getId());
					}
					if(result){
		...
		/*
		 * 捕获back按键，根据打钱的级别来判断，此时应该返回的市列表，生列表，还是直接退出
		 * 
		 */
		@Override
